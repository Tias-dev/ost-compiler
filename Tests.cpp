...
const char* help =
    ...
    "3. OPTIONAL: cursor position(counting from 0) at the end of program(if "
    "not specified it will be after last non space cell)\n"
    "4. Tests delimeter: line of any number of '-' symbols\n"
    "\n"
    "Output:\n"
    "Returns 0 if all tests are passed i.e. result line state equal expected "
    "line state for all tests \n"
...
void parseCommandArgs(int argc, char* argw[]) {
  ...
  int arg, longindex;
  while ((arg = getopt_long(argc, argw, "l:o:gdbh", options, &longindex)) !=
         -1) {
    switch (arg) {
      case '?':
...
int main(int argc, char* argw[]) {
  ...
  while (!testsFile.eof()) {
    ...
    for (auto& c : delimeter)
      if (c != '-')
        throw std::invalid_argument(
            strfast() << "Tests delimeter awaits to consist of '-' only, [" << c
                      << "] given");

    tu4run::Line<char> expectedLine{expected};
    if (cursor != -1) {
      while (cursor > expectedLine.cursor()) expectedLine.shiftRight();
      while (cursor < expectedLine.cursor()) expectedLine.shiftLeft();
    }
    ...
    if (runner.line() != expectedLine) {
      ...
      std::cout << "Expected: " << expectedLine.cursor() << ": ["
                << expectedLine.line() << "]" << std::endl;
      std::cout << "Given: " << runner.line().cursor() << ": ["
                << runner.line().line() << "]" << std::endl;
      wasErrors = true;
    } else {
