...
const char* helpMessage =
    ...
    "Name of output file is the name of first MT described in <program>.ost "
    "file\n"
    "\n"
...
void parseCommandArgs(int argc, char* argw[]) {
  ...
  int arg, longindex;
  while ((arg = getopt_long(argc, argw, "l:o:gdbhv", options, &longindex)) !=
         -1) {
    switch (arg) {
      case '?':
...
int main(int argc, char* argw[]) {
  ...
  compileAndSaveProgram(fileName, globals::libDir, globals::outDir,
