...
class Trie {
  ...
  class NodeIterator : public IActivePoint<TData> {
   ...
   public:
    ...
    const TData& get() const override { return node_->data(); }
  ...
  class Bimap {
   ...
   public:
    ...
    const std::string& toString(const TData& data) const {
      return toString_.at(data);
    }
  };
 ...
 public:
  ...
  bool contains(const std::string& s) { return find(s).has_value(); }
...
template <typename TData>
bool Trie<TData>::Node::isTerm() const {
  return data_.has_value();
}

template <typename TData>
bool Trie<TData>::Node::isLeaf() const {
  return tr_.size() == 0;
}
...
template <typename TData>
const TData& Trie<TData>::Node::data() const {
  if (!isTerm())
...
void Trie<TData>::add(const std::string& s, TData data) {
  ...
  for (auto& c : s) {
    if (!activePoint->canGoTo(c)) activePoint->addTranslation(c);
    activePoint->goTo(c);
  }
...
std::optional<TData>& Trie<TData>::find(const std::string& s) {
  ...
  point_type_ activePoint = begin_();
  for (auto& c : s) {
    if (!activePoint->canGoTo(c)) return nullopt;
    activePoint->goTo(c);
  }
  if (!activePoint->isTerm()) return nullopt;

  return activePoint->node()->data_opt();
}

template <typename TData>
typename Trie<TData>::bimap_type Trie<TData>::bimap() const {
  Bimap bimap;
  std::string buffer;
...
template <typename TData>
void Trie<TData>::bimap_(Node* node, Bimap& bimap, std::string& buffer) const {
  if (node->isTerm()) bimap.add(buffer, node->data());
