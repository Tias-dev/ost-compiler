...
class tu4_union {
 ...
 public:
  ...
  void updateEndState(TQ newState) {
    std::visit([&newState](auto& command) { command.updateEndState(newState); },
               *data_);
  }
...
tu4::tu4_union<TQ, TLetter> load(std::istream& is) {
  ...
  if (!isValidLine(line)) throw error::UnexpectedFileEnd();
  ...
  if (c2 == '>' || c2 == '<') {
    auto command = tu4::Tu4Move<TQ, TLetter>{
        ...
        false};
    if (globals::enableBreakpoints) command.setBreakpoint(range);
    return {command};
  } else {
