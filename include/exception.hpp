...
class UndefinedOperatorError : public PositionErrorBase {
 ...
 public:
  UndefinedOperatorError(const FilePosition& position)
      : PositionErrorBase(position) {}
...
class CommentNotClosedError : public PositionErrorBase {
 public:
  CommentNotClosedError(const FilePosition& position)
      : PositionErrorBase(position) {}
...
class BordersError : public std::exception {
 ...
 public:
  BordersError(const FilePosition& begin, const FilePosition& end)
      : begin_(begin), end_(end) {}

  const char* what() const noexcept override {
    static std::string message;
    message = (strfast() << "begin(" << begin_.to_string()
                         << ") of token must be less then end("
                         << end_.to_string() << ")");
...
class RedefinitionError : public PositionErrorBase {
 ...
 public:
  ...
  const char* what() const noexcept override {
    static std::string message;
    message = (strfast() << "Redefinition of mt: [" << name_ << "]; "
                         << position().to_string())
                  .bump();
...
class UnexpectedFileEnd : public std::exception {
 ...
 public:
  ...
  const char* what() const noexcept override {
    static std::string message;
    message =
        (strfast() << "Unexpected end of file"
                   << (expected_.size() > 0 ? ". Expected: " + expected_ : ""))
            .bump();
    return message.c_str();
  }
};
